    ...
    >
      ...
      <div className="flex items-center justify-between px-5 pt-5">
        <div className="flex items-center gap-2">
          <h3 className="text-base font-semibold text-slate-800">SIKORA AI</h3>
            ...
            <div className="mt-4 space-y-2">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => ask(s)}
                  className="flex w-full items-center justify-between gap-2 rounded-xl bg-white/80 px-3.5 py-2.5 text-left text-sm text-slate-600 shadow-sm ring-1 ring-slate-100 transition-colors hover:bg-white hover:text-slate-900"
                >
                  {s}
      ...
      >
        <input
          ...
          className="flex-1 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm outline-none placeholder:text-slate-400 focus:border-primary"
        />
        <button
          type="submit"
